...
import {
  ...
  AlertCircle, Cpu, Database, Globe, ChevronDown
} from 'lucide-react';
...
// ── Streaming message renderer (single render, no Typewriter duplication) ──
...
const ScrollToBottomButton = ({
  ...
  return (
    <button
      ...
      className="fixed bottom-32 right-8 z-40 flex items-center gap-1.5 px-3 py-2 bg-[#121418] border border-[#00FF85]/40 text-[#00FF85] text-xs font-bold uppercase tracking-wider hover:bg-[#00FF85]/10 transition-all shadow-lg shadow-black/40 animate-bounce"
...
export default function StockMindChat() {
  ...
  // ── Get ALL file IDs (staged + previously uploaded) ────────────────────
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    // ✅ Capture ALL file IDs BEFORE any state mutations
  ...
  const handleFileAttach = (e: React.ChangeEvent<HTMLInputElement>) => {
    Array.from(e.target.files || []).forEach(f => uploadHook.upload(f));
            ...
            <div className="space-y-1">
              {session.sessions.map((s, idx) => (
                <motion.button
                  ...
                  className={cn(
                    "w-full flex flex-col items-start p-3 transition-all hover:bg-white/5 border-l-2",
          ...
          <div>
            ...
            {totalSessionFiles > 0 && (
              <p className="text-[9px] text-[#00FF85]/40 mt-3 px-2 leading-relaxed">
                📎 All files included automatically in every query.
              </p>
            )}
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col relative">
        ...
        <header className="h-12 border-b border-white/5 flex items-center justify-between px-6 bg-[#0A0C10]/80 backdrop-blur-md z-30">
          <div className="flex items-center gap-4">
            <h2 className="text-sm font-bold truncate max-w-[160px]">
              {session.sessions.find(s => s.id === session.activeSessionId)?.name || 'Untitled Session'}
            </h2>
            <div className="h-4 w-[1px] bg-white/10" />
            <div className="flex items-center gap-2">
              {[
                { key: 'data', icon: <Database size={10} />, label: 'Data Agent', active: agentStatus.data, idle: false },
                { key: 'research', icon: <Globe size={10} />, label: 'News Agent', active: agentStatus.research, idle: false },
                {
                  key: 'rag', icon: <Cpu size={10} />,
                  label: `RAG Agent${totalSessionFiles > 0 && !agentStatus.rag ? ` (${totalSessionFiles})` : ''}`,
                  active: agentStatus.rag, idle: totalSessionFiles > 0,
                },
        ...
        {/* ── CHAT AREA ── ref={scrollRef} enables smart scroll ──────── */}
        <div
          ref={scrollRef}
        ...
        >
            ...
            {isDragging && (
              ...
              >
                <div className="w-full h-full border-2 border-dashed border-[#00FF85] flex flex-col items-center justify-center gap-4 bg-[#00FF85]/5">
                  ...
                  <p className="text-xl font-bold uppercase tracking-widest text-[#00FF85]">
                    Drop files to research
                  </p>
                </div>
              </motion.div>
            )}
          ...
          {messages.map((msg, idx) => (
            <React.Fragment key={msg.id}>
              ...
              {msg.role === 'assistant' && idx === messages.length - 1 &&
                (isAgentStreaming || thoughtSteps.length > 0) && (
          ...
          <ScrollToBottomButton scrollToBottom={scrollToBottom} isNearBottomRef={isNearBottomRef} />
        </div>
              ...
              {chat.hitlPending && (
                ...
                >
                  ...
                  <p className="text-sm mb-4 text-white/70">
                    I resolved{' '}
                    <span className="font-mono font-bold text-white">"{chat.hitlRawInput}"</span>
                    {' → '}
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter')
                            handleTickerReject((e.target as HTMLInputElement).value);
                        }}
                      />
                      <button
                        onClick={() => {
